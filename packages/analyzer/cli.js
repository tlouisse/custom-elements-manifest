...
export async function cli() {
  ...
  if (mainOptions.command === 'analyze') {
    const { config: configPath, ...cliConfig } = getCliConfig(argv);
    ...
    async function run() {
      const modules = userConfig?.overrideModuleCreation
        ...
        : globs.map((glob) => {
            ...
            return ts.createSourceFile(relativeModulePath, source, ts.ScriptTarget.ES2015, true);
          });
      ...
      if (mergedOptions?.dependencies) {
        try {
          thirdPartyCEMs = await findExternalManifests(globs);
