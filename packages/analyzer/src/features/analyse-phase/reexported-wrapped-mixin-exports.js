          ...
          if (!isMixin(node)) {
            node?.declarationList?.declarations?.forEach((declaration) => {
              ...
              if (ts.SyntaxKind.CallExpression === declaration?.initializer?.kind) {
                 ...
                 */
                const moduleMixinDeclarations = moduleDoc?.declarations?.filter(
                  (declaration) => declaration.kind === 'mixin',
                );
                ...
                if (has(moduleMixinDeclarations)) {
                   ...
                   */
                  const foundMixin = moduleMixinDeclarations?.find(
                    (mixin) => mixin.name === node?.getText(),
                  );
                  if (foundMixin) {
                    ...
                    moduleDoc.declarations = moduleDoc?.declarations?.filter(
                      (declaration) =>
                        !(declaration.kind === 'variable' && declaration.name === foundMixin.name),
                    );
                  }
                }
              }
