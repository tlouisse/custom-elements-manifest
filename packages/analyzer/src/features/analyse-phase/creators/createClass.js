 ...
 * @typedef {import('typescript').Statement} TsStatement
 * @typedef {import('typescript').ConstructorDeclaration} TsConstructorDeclaration
 * @typedef {import('custom-elements-manifest/schema').JavaScriptModule} JavaScriptModule
 * @typedef {import('custom-elements-manifest/schema').CustomElementDeclaration} CustomElementDeclaration
  ...
  node?.members?.forEach((member) => {
    ...
    if (isProperty(member) && !hasIgnoreJSDoc(member)) {
      ...
      if (hasAttrAnnotation(member)) {
         ...
         */
        let attrAlreadyExists = classTemplate.attributes.find(
      ...
      if (field?.static) {
        ...
        if (fieldExists) {
          classTemplate.members = classTemplate.members.map((mem) =>
            mem?.name === member?.name?.getText() && !mem?.static ? { ...mem, ...field } : mem,
          );
        } else {
          classTemplate.members.push(field);
        }
...
/**
 * @param {TsClassDeclaration|TsConstructorDeclaration} source
 * @param {CemClassTemplate} classTemplate
 * @param {PluginContext} context
 */
export function getDefaultValuesFromConstructorVisitor(source, classTemplate, context) {
  visitNode(/** @type {TsConstructorDeclaration} */ (source));

  function visitNode(/** @type {TsConstructorDeclaration} */ node) {
    switch (node.kind) {
      case ts.SyntaxKind.Constructor:
        ...
        node.body?.statements
          ...
          .forEach((statement) =>
            mapClassMember(source, classTemplate, context, node, statement, statement.expression),
          );
 ...
 * @param {PluginContext} context
 * @param {Node} node
 * @param {TsStatement} statement
 * @param {TsExpressionStatement} expression
 */
function mapClassMember(source, classTemplate, context, node, statement, expression) {
  let existingMember = classTemplate?.members?.find(
    (member) => expression?.left?.name?.getText() === member.name && member.kind === 'field',
  );
    ...
    if (expression?.right?.kind === ts.SyntaxKind.Identifier) {
      existingMember.resolveInitializer = {
        ...resolveModuleOrPackageSpecifier(
          { path: source.getSourceFile().fileName },
          context,
